namespace ANG24.Core.Entities.DataTypes
{
    public class KP500ControllerData
    {
        ...
        public void SetModeMCH2(KP500SetMCH2 setMCH2)
        {
             ...
             *  26, 27, 28 -- уровень
             *  42, 43, 44 -- частота
             *  58, 59, 60 -- непрерывный, импульсный
             *  74, 75, 76 -- диапазоны нагрузки
             *  90, 91, 92 -- переход в дополнительные экраны
             *  
             * [3] - код bMode
             *  xxxx xx00 - 480Hz                           (0x00)
             *  xxxx xx01 - 1069Hz                          (0x01)
             *  xxxx xx10 - 9796Hz                          (0x02)
             *  xxxx 1xxx - Auto mode                       (0x08)
             *  xxxx 0xxx - Fix mode
             *  xxxx x011 - изменение частоты 1             (0x03)
             *  xxxx x100 - изменение частоты 1             (0x04)
             *  xxxx x101 - изменение частоты 1             (0x05)
             *  0000 xxxx - 0.5 ohm                         (0x00)
             *  0001 xxxx - 1 ohm                           (0x10)
             *  0010 xxxx - 2 ohm                           (0x20) 
             *  0011 xxxx - 4 ohm                           (0x30)
             *  0100 xxxx - 8 ohm                           (0x40)
             *  0101 xxxx - 16 ohm                          (0x50)
             *  0110 xxxx - 32 ohm                          (0x60)
             *  0111 xxxx - 64 ohm                          (0x70)
             *  1000 xxxx - 128 ohm                         (0x80)
             *  1001 xxxx - 256 ohm                         (0x90)
             * [4] - bState
             *  xxxx xxx1 - автономный режим                (0x01)
             *  xxxx xxx0 - дист. режим                     (0x00)
             *  xxxx xx1x - RS-232                          (0x02)
             *  xxxx xx0x - RS-485
             *  x1xx xxxx - ВКЛ Ер                          (0x40)
             *  x0xx xxxx - ВЫКЛ Ер
             *  1xxx xxxx - ВКЛ Usin                        (0x80)
             *  0xxx xxxx - ВЫКЛ Usin
             *  xx0x xxxx - Признак паузы в имп. режиме
             *  xx1x xxxx - Признак импульса в имп. режиме  (0x20)
             * [5] - 
             * [6]
             * [7]
             * [8]
            ...
            while (!result || resetAll)
            {
                ...
                switch (step)
                {
                    case 0:
                        // Чтение текущего состояния
                        request = GetCurrentInfo(); //0x01 (01)
                        if (request.state == false) return;
                        step = 1;
                        //countError = 0;
                        break;
                    case 1:
                        // Переход в экран смены частоты
                        if (request.value[2] == 43) //если это экран смены частоты
                        {
                            step = 3;
                            countError = 0;
                        }
                        ...
                        {
                            oldMode = request.value[2]; //запомнить текущий экран
                            ButtonMode();               //нажать "реж"
                            Thread.Sleep(200);
                            step = 2;
                        }
                        break;
                    case 2:
                        ...
                        countError++;
                        if (countError > 50) return;
                        if (oldMode == request.value[2] || request.value[2] == 10) //если остался на том же экране или находится на основном
                        {
                            Thread.Sleep(200); //зацикливает пока не заполнится счетчик ошибок(это считат ошибкой) или пока состояние не изменится
                        }
                        else
                        {
                            step = 1; //если состояние изменилось -- на шаг 1
                        }
                        break;
                    case 3:
                        // Изменение режима
                        request = GetCurrentInfo(); //0x01 (01)
                        if (request.state == false) return;
                        // Проверка что это экран переключение частоты
